...
def home():
    ...
    c.execute("SELECT subject, COUNT(DISTINCT date) as total_classes FROM attendance GROUP BY subject")
    subject_data = c.fetchall()

    # Get the defaulters list
    c.execute("SELECT students.name, students.roll_no, attendance.subject, (SUM(CASE WHEN attendance.status = 'Present' THEN 1 ELSE 0 END) * 100.0 / COUNT(attendance.id)) as attendance_percentage FROM students INNER JOIN attendance ON students.id = attendance.student_id GROUP BY students.id, attendance.subject HAVING attendance_percentage < 75")
...
def mark_attendance():
    if request.method == 'POST':
        ...
        c.execute("SELECT COUNT(*) FROM attendance WHERE date = ? AND subject = ?", (date, subject))
